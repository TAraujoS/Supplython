...
def factory_pdf(self, contract, employee):
    ...
    pdf.ln(10)
    pdf.multi_cell(txt=f"Invoice_number : {self['invoice_number']}", w=0)
    pdf.ln(2)
    ...
    pdf.ln(2)
    pdf.multi_cell(txt=f"Telefone : {contract.supplier.tel}", w=0)
    pdf.ln(2)
    ...
    pdf.ln(2)
    pdf.multi_cell(txt=f"Department_id : {employee.department_id}", w=0)
    pdf.ln(2)
